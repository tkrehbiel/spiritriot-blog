...
async function getFile(filename) {
    let dirname = process.env.EGV_USER_DATA_LOCATION;
    if (!dirname || !dirname.startsWith('file://')) dirname = 'data';
    else dirname = dirname.substring(7);
    const pathname = [process.cwd(), dirname, filename].join('/');
    try {
